...
namespace SixtyThreeBits.Web.Domain
{
    public class BeforeWebProjectControllerLoaded : IAsyncActionFilter
    {
        ...
        public BeforeWebProjectControllerLoaded(AppSettingsCollection appSettings, UtilityCollection utilities, RepositoryFactory dataAccessFactory)
        {
        ...
        async Task InitUser()
        {
            _model.User = _model.SessionAssistance.Get<UserDTO>(Constants.Session.User);

            if (_model.User == null)
            {
                var userID = _model.CookieAssistance.Get<int?>(Constants.Cookies.User);
                if (userID != null)
                {
                    var repository = _dataAccessFactory.GetUsersRepository();
                    _model.User = await repository.UsersGetSingleUserByUserID(userID);
                    _model.SessionAssistance.Set(Constants.Session.User, _model.User);
                }
            }
        }
