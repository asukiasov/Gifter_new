...
namespace SixtyThreeBits.Web.Admin.Models
{
    public class UsersModel : WebProjectModelBase
    {
        ...
        public async Task<List<PageViewModel.GridModel.GridItem>> GetGridViewModel()
        {
            ...
            var viewModel = (await repository.UsersList())?.Select(Item => new PageViewModel.GridModel.GridItem
            {
                ...
                UrlUserProperties = Url.RouteUrl(ControllerActionRouteNames.Admin.UserManagement.User.Properties, new { userID = Item.UserID }),
            }).ToList();
            return viewModel;
        }
    ...
    public class UserModelBase : WebProjectModelBase
    {
        #region Properties
