...
namespace SixtyThreeBits.Web.Filters.Admin
{
    public class NewsFilterAttribute : IAsyncActionFilter
    {
        ...
        public async Task OnActionExecutionAsync(ActionExecutingContext filterContext, ActionExecutionDelegate next)
        {
            ...
            _model.NewsItem = await repository.NewsGetSingleByID(newsID);
            if (_model.NewsItem == null)
            {
        ...
        void initPageTitle()
        {
            _model.PageTitle.Set(_model.NewsItem.NewsTitle);
        }

        void reinitBreadCrumbs()
        {
            _model.Breadcrumbs.DeleteLastItem();
            _model.Breadcrumbs.RenameLastItem(_model.NewsItem.NewsTitle);
        }
        #endregion
    }
}